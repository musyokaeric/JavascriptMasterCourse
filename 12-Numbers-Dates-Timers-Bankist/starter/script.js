...
const formatCurr = function (locale, currency, movement) {
...
const displayMovements = function (acc, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    const formattedMovement = formatCurr(acc.locale, acc.currency, mov);
...
const calcDisplaySummary = function (acc) {
  ...
  const out = acc.movements
    .filter(mov => mov < 0)
    .reduce((acc, mov) => acc + mov, 0);
  labelSumOut.textContent = formatCurr(acc.locale, acc.currency, out);
